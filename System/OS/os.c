...
static uint MenuTaskStack[MenuTaskStackSum];            //菜单界面任务栈数组
...
#ifndef __ONLINE_DEBUG
  System.Device.Iwdog.IwdogStart(2500); // set Iwatchdog per 2000ms delay
#endif
...
static uint PendMessageQueue(void)
{
    ...
    EnterCritical();
    message = LogicTask.Message; 	        // 读取单次消息，业务逻辑激活后从这儿开始执行
    LogicTask.Message = 0; 	
    ExitCritical();
    return (message);
}
